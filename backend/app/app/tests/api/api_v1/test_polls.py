...
from app.tests.utils.poll import get_random_poll
from app.tests.utils.user import authentication_token_from_username, get_random_user
...
def test_create_poll_invalid_inputs(
    ...
    # duplicate poll id
    with get_random_poll(db, owner_id=current_user["id"]) as existing_poll:
        data = copy.deepcopy(base_data)
...
def test_voting_numerai_stake(
    ...
    with get_random_poll(
        ...
        model_name = "zzzzz" + random_lower_string()
        ...
        # second vote
        with get_random_user(db) as another_voter:
            ...
            another_model_name = "zzzzz" + random_lower_string()
    ...
    with get_random_poll(
        ...
        with get_random_user(db) as another_voter:
            another_voter = crud.user.update(
                ...
                obj_in={
                    "is_superuser": True,
                    "numerai_wallet_address": another_voter.username,
                },
            )
            another_token_headers = authentication_token_from_username(
                client=client, username=another_voter.username, db=db
            ...
            # close poll
            response = client.post(
                f"{settings.API_V1_STR}/polls/{poll.id}/close",
                headers=superuser_token_headers,
            )
            assert response.status_code == 200
            content = response.json()
