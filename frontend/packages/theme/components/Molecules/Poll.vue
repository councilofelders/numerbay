      ...
      <div v-for="(a,index) in calcAnswers" :key="index" :class="{ ans: true, [a.custom_class]: (a.custom_class) }">
        ...
        <template v-if="!finalResults">
          ...
          <div v-else :class="{ 'ans-voted': true, selected: a.selected }">
            <span v-if="canShowResults && a.percent" class="percent" v-text="a.percent"></span>
            <span class="txt" v-html="a.text"></span>
          </div>

          <span class="bg" :style="{ width: visibleResults ? a.percent : '0%' }" v-if="canShowResults"></span>
        </template>
        <template v-else>
...
.vue-poll .ans-cnt .ans-no-vote {
  text-align: center;
  border: 2px solid var(--c-primary-lighten);
...
.vue-poll .ans-cnt .ans .bg.selected {
  background-color: var(--c-primary-lighten);
}
