      ...
      <div id="myTabContent" class="tab-content mt-4">
        <div id="account-information" aria-labelledby="account-information-tab" class="tab-pane fade show active"
          ...
          <div class="profile-setting-panel">
            ...
            <!-- Temporary pricing discount factor -->
            <hr class="my-4">
            <h5 class="mb-4">(Optional) Pricing adj. factor on May 1 2023</h5>
            <div class="alert alert-warning d-flex mb-4" role="alert">
              ...
              <p class="fs-14">
                Daily tournament payout starts on May 1 2023 <a href="https://forum.numer.ai/t/daily-tournament-update-2-payouts/6189" target="_blank">(Read More)</a>.
                <br>This one-time pricing adjustment factor will be applied to all of your active products on that day.
                <br><i>Set to 0.2 for the equivalent of current pricing (5 daily rounds vs. 1 weekend round currently).</i>
              </p>
            </div><!-- end alert -->
            <ValidationObserver key="factor" v-slot="{ handleSubmit }">
              <div class="row mt-4">
                <ValidationProvider v-slot="{ errors }" rules="required" slim>
                  <div class="col-lg-6 mb-3">
                    <input id="factor" v-model="form.factor" :class="!errors[0] ? '' : 'is-invalid'"
                           class="form-control form-control-s1" required type="text" placeholder="0 to 1 (e.g. 0.2), 1 means unchanged">
                    <div :class="{ 'show': Boolean(errors[0]) }" class="text-danger fade">{{ errors[0] }}</div>
                  </div><!-- end col -->
                </ValidationProvider>
              </div><!-- end row -->
              <button :disabled="userLoading" class="btn btn-dark mt-3 d-flex justify-content-center"
                      type="button" @click="handleSubmit(onSaveFactor)">
                <span v-if="userLoading"><span class="spinner-border spinner-border-sm me-2" role="status"></span>Saving...</span>
