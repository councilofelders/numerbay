<template>
    <div class="col-lg-8">
        ...
        <div class="alert alert-danger d-flex mb-4" role="alert" v-if="!userGetters.getNumeraiApiKeyPublicId(user)">
            ...
            <p class="fs-14">
              In order to list Numerai products for sale, you must <router-link to="/numerai-settings" class="btn-link">set up a Numerai API key.</router-link> It only takes a few minutes!
            </p>
        </div><!-- end alert -->
        <div class="profile-setting-panel-wrap">
          <router-link :to="`/create-listing`" class="btn btn-outline-dark mb-4" :class="(!userGetters.getNumeraiApiKeyPublicId(user) || userLoading) ? 'disabled' : ''"><em class="ni ni-plus"></em> New Listing</router-link>
          <div class="table-responsive">
                <table class="table mb-0 table-s2">
                    ...
                    <tbody class="fs-13">
                        <tr v-if="!products || products.length === 0"><td colspan="3" class="text-secondary">You currently have no listing</td></tr>
