<template>
  <div class="col-lg-8">
    ...
    <div class="profile-setting-panel-wrap">
      <router-link :to="`/create-poll`" class="btn btn-outline-dark mb-4"
      ...
      <div class="table-responsive">
        <table class="table mb-0 table-s2">
          ...
          <tr v-if="!polls || polls.length === 0">
            <td colspan="3" class="text-secondary">You have not created any poll</td>
          </tr>
          <tr v-for="poll in polls" :key="pollGetters.getId(poll)">
            <th scope="row">
              <router-link class="btn-link" :to="`/vote/${pollGetters.getId(poll)}`"><span class="text-break"
              ...
              <div class="d-flex justify-content-between">
                <button class="icon-btn ms-auto me-2" title="Close" @click="handleClosePoll(pollGetters.getId(poll))">
                  <span class="spinner-border spinner-border-sm text-secondary" role="status"
                        v-if="Boolean(polls) && pollLoading"></span>
