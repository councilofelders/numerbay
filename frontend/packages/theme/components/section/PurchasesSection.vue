      ...
      <div class="table-responsive">
        <table class="table mb-0 table-s2">
          ...
          <tr v-if="!displayedOrders || displayedOrders.length === 0">
            <td colspan="3" class="text-secondary">You have not made any purchase</td>
          </tr>
          <tr v-for="order in displayedOrders" :key="order.id">
            <th scope="row"><a href="javascript:void(0);" @click="toggleInfoModal(order)"
            ...
            <td>
              <div class="d-flex justify-content-between" v-if="orderGetters.getStatus(order)=='confirmed'">
                <button class="icon-btn ms-auto" title="Download Artifacts" @click="toggleArtifactModal(order)"><em
